...
if (continue_progress) {
  ...
  dat_files <- list.files("data/output",pattern="^formed_data_[0-9]{4}[_0-9]+.RDS")
  
  dat_times <- dat_files %>%
    gsub("formed_data_","",.) %>%
    gsub(".RDS","",.) %>%
    strptime("%Y_%m_%d_%H_%M_%S")
  
  alldat <- dat_files %>%
    paste0("data/output/",.) %>%
    map2(dat_times,~readRDS(.x) %>% mutate(scrape_time=.y)) %>%
    bind_rows() %>%
    # mutate(filetype=gsub(".*\\.([a-z]{3,4})$","\\1",tolower(file)))
    {.}

  # Filter out duplicates from multiple scraping
  dat_to_keep <- alldat %>%
    select(date_year,date_month,org_body,scrape_time) %>%
    unique() %>%
    group_by(date_year,date_month,org_body) %>%
    mutate(keep=scrape_time==max(scrape_time)) %>%
    filter(keep==TRUE) %>%
    select(-keep) %>%
    ungroup()
  
  rawdat <- left_join(dat_to_keep,alldat)
  ...
  dat2 %>%
    ...
    select(Year,Month,Body,Department,org_type,depmatch,org_main,org_body) %>%
    unique() %>%
  ...
  dat4 %>%
    filter(duples>1) %>%
    select(file,org_type,Month,Year,Body,Department,duples) %>%
    unique() %>%
    arrange(Department,Body,Year,Month) %>%
    write.csv("data/gov_errors/file_duplication.csv",row.names=FALSE)
  # Drop all data where there are extra files
  # TODO: track modification date and use that?
  dat5 <- dat4 %>%
